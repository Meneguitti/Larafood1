...
<div class="form-group">
    ...
    <input type="date" name="subscription" class="form-control" placeholder="Data de Assinatura" value="{{ $tenant->subscription ?? old('cnpj') }}">

</div>

<div class="form-group">
    <label>* Expira (Final):</label>
    <input type="date" name="expires_at" class="form-control" placeholder="Data de Expiração" value="{{ $tenant->expires_at ?? old('expires_at') }}">
</div>
...
<div class="form-group">
    ...
    <select name="subscription_active" class="form-control">
        ...
        <option value="2" @if(isset($tenant) && !$tenant->subscription_active) selected @endif>Não</option>
    </select>
</div>

<div class="form-group">
    <label>* Assinatura Cancelada?</label>
    <select name="subscription_suspendend" class="form-control">
        <option value="1" @if(isset($tenant) && $tenant->subscription_suspendend) selected @endif>Sim</option>
        <option value="2" @if(isset($tenant) && !$tenant->subscription_suspendend) selected @endif>Não</option>
    </select>
</div>
